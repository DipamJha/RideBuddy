          ...
          <motion.div variants={fadeUp} className="mb-8">
            <button
              ...
              onClick={async () => {
                ...
                setError("");
                try {
                  // Pre-warm the backend before redirecting to avoid Render cold-start splash
                  await fetch(`${backendUrl}/api/health`, { mode: "cors" });
                } catch {
                  // Even if health check fails, proceed with the redirect
                }
                window.location.href = `${backendUrl}/api/auth/google${tgChatId ? `?tg_chat_id=${tgChatId}` : ""}`;
              }}
              disabled={loading}
              className="w-full glass-card flex items-center justify-center gap-3 py-3.5 rounded-xl hover:border-primary/30 font-medium text-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            >
